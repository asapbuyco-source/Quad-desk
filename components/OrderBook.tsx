  ...
  return (
    ...
    `}>
      ...
      <div className="flex items-center gap-3 z-10 w-24">
        <span className={`font-bold tracking-tight flex items-center gap-2 ${isWall ? (isBid ? 'text-emerald-300' : 'text-rose-300') :
          ...
          {isWall && (
            <span className={`inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[9px] font-black tracking-widest border ${isBid
                ? 'bg-emerald-500/20 border-emerald-500/50 text-emerald-300'
                : 'bg-rose-500/20 border-rose-500/50 text-rose-300'
              }`}>
              <BoxSelect size={8} strokeWidth={3} />
              WALL
            </span>
          )}
      ...
      <div className="flex items-center justify-end gap-3 z-10 min-w-[100px]">
        ...
        {/* Size */}
        <span className={`transition-colors text-right w-12 ${isWall ? 'text-white font-black text-sm drop-shadow-md' :
